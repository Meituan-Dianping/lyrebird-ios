  ...
  <div v-else>
    <CellGroup @on-click="onCellClick">
      ...
      >
        ...
        <span style="padding-left:20px;font-size:12px;color:#bbbbbb">{{device.device_name}}</span>
        <Icon type="md-camera" slot="extra" size="18" @click.native="takeScreenShot(device.device_name)"/>
      </Cell>
    </CellGroup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    takeScreenShot (deviceName) {
      if (this.focusDeviceId) {
        this.$bus.$emit('msg.info', 'Taking screenshot of ' + deviceName)
        this.$store.dispatch('takeScreenShot')
      }
    }
  }
